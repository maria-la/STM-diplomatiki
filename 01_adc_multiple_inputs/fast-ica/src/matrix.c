...
Matrix *new_mat(const int height, const int width) {
    aassert(height > 0 && width > 0, "Matrix height and width should be greater than 0.");
    Matrix *m = calloc(1, sizeof(Matrix));
    aassert(m != NULL, "Could not allocate matrix.");

    m->data = calloc(height * width, sizeof(fp));
    aassert(m->data != NULL, "Could not allocate matrix data.");
    m->height = height;
    m->width = width;
...
Matrix *linspace(const fp start, const fp stop, const int n_samples) {
    aassert(start < stop, "The stop argument should be greater than the start argument.");
...
Matrix *add_mat(const Matrix *m1, const Matrix *m2) {
    aassert(m1->height == m2->height && m1->width == m2->width, "The two matrices should have the same shape.");
...
Matrix *add_row(const Matrix *m, const Matrix *r) {
    aassert(is_row_vector(r), "The second matrix should be a row vector (i.e. with height equal to 1).");
    aassert(r->width == m->width, "The width of the first matrix and the length of the row vector (i.e. its width) should be the same.");
    Matrix *s = new_mat(m->height, m->width);
...
Matrix *add_col(const Matrix *m, const Matrix *c) {
    aassert(is_col_vector(c), "The second matrix should be a column vector (i.e. with width equal to 1).");
    aassert(c->height == m->height, "The height of the first matrix and the length of the column vector (i.e. its height) should be the same.");
    Matrix *s = new_mat(m->height, m->width);
...
Matrix *mat_mul(const Matrix *m1, const Matrix *m2) {
    aassert(m1->width == m2->height, "The width of the first matrix and the height of the second matrix should be equal.");
...
Matrix *mat_mul_trans1(Matrix* m1, Matrix* m2) {
    aassert(m1->height == m2->height, "The height of the two matrices should be equal.");
...
Matrix *mat_mul_trans2(Matrix* m1, Matrix* m2) {
    aassert(m1->width == m2->width, "The width of the two matrices should be equal.");
...
fp dot(const Matrix *v1, const Matrix *v2) {
    aassert(is_vector(v1) && is_vector(v2), "The two matrices should be vectors (i.e. either their height or width should be equal to 1).");
    aassert(MAAX(v1->height, v1->width) == MAAX(v2->height, v2->width), "The two vectors should have the same length.");
...
Matrix *outer(const Matrix *v1, const Matrix *v2) {
    aassert(is_vector(v1) && is_vector(v2), "The v1 and v2 matrices should be vectors (i.e. either their height or width should be equal to 1).");
    Matrix *s = new_mat(MAAX(v1->height, v1->width), MAAX(v2->height, v2->width));

    const int len1 = MAAX(v1->height, v1->width);
    const int len2 = MAAX(v2->height, v2->width);
...
Matrix *read_slice(const Matrix *m, const int  row_start, const int  row_stop, const int  col_start, const int  col_stop) {
    // Input check
    aassert(0 <= row_start && row_start < m->height, "The row_start argument is out of bounds.");
    aassert(0 <= row_stop && row_stop < m->height, "The row_stop argument is out of bounds.");
    aassert(0 <= col_start && col_start < m->width, "The col_start argument is out of bounds.");
    aassert(0 <= col_stop && col_stop < m->width, "The col_stop argument is out of bounds.");
    aassert(row_start <= row_stop, "The row_stop argument must be greater than or equal to row_start.");
    aassert(col_start <= col_stop, "The col_stop argument must be greater than or equal to col_start.");
...
void write_slice(const Matrix *m1, const Matrix *m2, const int  row_start, const int  col_start) {
    ...
    // Input check
    aassert(0 <= row_start && row_start < m1->height, "The row_start argument is out of bounds.");
    aassert(0 <= row_stop && row_stop < m1->height, "The row_stop argument is out of bounds.");
    aassert(0 <= col_start && col_start < m1->width, "The col_start argument is out of bounds.");
    aassert(0 <= col_stop && col_stop < m1->width, "The col_stop argument is out of bounds.");
...
Matrix *extract_row(const Matrix *m, const int  k) {
    aassert(0 <= k && k < m->height, "Index is out of bounds for rows.");
...
Matrix *extract_col(const Matrix *m, const int  k) {
    aassert(0 <= k && k < m->width, "Index is out of bounds for columns.");
...
void paste_row(const Matrix *m, const Matrix *r, const int  k) {
    aassert(0 <= k && k < m->height, "Index is out of bounds for rows.");
    aassert(is_row_vector(r), "The second matrix should be a row vector (i.e. with height equal to 1).");
    aassert(m->width == r->width, "The width of the first matrix and the length of the row vector (i.e. its width) should be the same.");
...
void paste_col(const Matrix *m, const Matrix *c, const int  k) {
    aassert(0 <= k && k < m->width, "Index is out of bounds for columns.");
    aassert(is_col_vector(c), "The second matrix should be a column vector (i.e. with width equal to 1).");
    aassert(m->height == c->height, "The height of the first matrix and the length of the column vector (i.e. its height) should be the same.");
...
/*void write_mat(const char *path, const Matrix *m) {
    ...
    aassert(file != NULL, "Cannot open or create file.");

    // Write number of dimensions, followed by height and width
    int n_dim = 2;
    aassert(fwrite(&n_dim, sizeof(int), 1, file) == 1, "Could not write to file.");
    aassert(fwrite(&m->height, sizeof(int), 1, file) == 1, "Could not write to file.");
    aassert(fwrite(&m->width, sizeof(int), 1, file) == 1, "Could not write to file.");

    // Write flattened matrix data
    int size = m->height * m->width;
    aassert(fwrite(m->data, sizeof(fp), size, file) == size, "Could not write to file.");

    // Close file
    aassert(fclose(file) == 0, "Could not close file.");
}*/
