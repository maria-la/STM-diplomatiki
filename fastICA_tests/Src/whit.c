...
void whitenRows(arm_matrix_instance_f32 *matrix, arm_matrix_instance_f32 *matrixOut){
    arm_matrix_instance_f32 matT, covm, U, S, U1, T, Ut;
    ...
    float32_t Sdata[numRows*numRows];
    float32_t Vdata[numRows*numRows];
    ...
    arm_mat_init_f32 (&S, numRows, numRows, Sdata);
    arm_mat_init_f32 (&T, numRows, numRows, Tdata);

    arm_mat_trans_f32 (matrix, &matT);
    covMat(matTdata, covData, numRows, numRows);

    svd_golub_reinsch(covData, numRows, numRows, Udata, sigmaDataDiag, Vdata);
    ...
    arm_mat_mult_f32(&T, matrix, matrixOut);
